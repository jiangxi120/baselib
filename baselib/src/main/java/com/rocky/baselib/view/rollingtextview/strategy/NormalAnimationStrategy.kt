...
open class NormalAnimationStrategy : SimpleCharOrderStrategy() {
    ...
    override fun findCharOrder(
        ...
        return if (sourceChar == targetChar) {
            listOf(targetChar) to Direction.SCROLL_UP

        } else if (order == null) {
            listOf(sourceChar, targetChar) to Direction.SCROLL_UP
            ...
            if (srcIndex < tgtIndex) {
                ...
                order.subList(tgtIndex, srcIndex).asReversed() to Direction.SCROLL_DOWN
            }
        }
    }
