...
import com.rocky.baselib.view.rollingtextview.TextManager
import com.rocky.baselib.view.rollingtextview.util.CircularList
import com.rocky.baselib.view.rollingtextview.util.ReplaceList
...
open class NonZeroFirstStrategy(private val strategy: CharOrderStrategy) : CharOrderStrategy by strategy {
    ...
    override fun findCharOrder(
        ...
        var replaceList = if (replaceFirst && replaceLast) {
            ...
            ReplaceList(list, firstItem = TextManager.EMPTY, firstReplacePosition = { firstIdx },
                    lastReplacePosition = { lastIdx })
        } else if (replaceLast) {
            ReplaceList(list, lastItem = TextManager.EMPTY, firstReplacePosition = { firstIdx },
                    lastReplacePosition = { lastIdx })
        } else {
            list
        }
